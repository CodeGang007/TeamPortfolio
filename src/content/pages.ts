...
export const services: PageSpec[] = [
  ...
  },
  {
    ...
    plate: "Analytics: one report, three panels",
    offerings: [
      { title: "Report and dashboard design", body: "The page set, the drill path, and what a reader should be able to answer within ten seconds of opening it." },
      { title: "Semantic models and DAX", body: "Every measure defined once, so Total Sales means the same thing in every panel and every meeting." },
      { title: "ETL pipelines", body: "Scheduled Azure Data Factory jobs that land clean, typed data instead of somebody re-exporting a CSV on Monday." },
      { title: "Source integrations", body: "Salesforce, the ERP, the school MIS, the point of sale: read at the source rather than through an export." },
      { title: "Data quality checks", body: "Rows that fail validation surface inside the report, not silently inside an average." },
      { title: "Handover and training", body: "The people who read the report can add a measure and publish it without opening a ticket." },
    ],
    outcomes: [
      { lead: "One version of the number", body: "Sales, operations and finance open the same report on the same definitions, so the meeting starts at the decision." },
      { lead: "You see it while it matters", body: "Service level, attendance, stock movement: measured on a rolling window, so a bad day is visible while it is still happening." },
      { lead: "The pipeline is yours", body: "It runs in your Microsoft tenant under your licences, documented well enough that another team could pick it up." },
    ],
    stack: ["Power BI", "DAX", "Power Query", "Azure Data Factory", "Azure SQL Database", "Salesforce"],
    proof: ["sales-performance-report", "call-centre-dashboard", "local-shops-analytics", "student-insights-suite"],
    faq: [
      { q: "Do we need a data warehouse first?", a: "Not always. Two of the four systems on this page read their sources directly. We build a warehouse when the history, the volume, or the number of sources makes it cheaper than not having one, and we tell you which case you are in before you spend anything." },
      { q: "Can you work with the Power BI we already have?", a: "Yes, and it is where most of this work starts: reports that exist but nobody trusts. We usually rebuild the semantic model underneath before touching a single visual, because that is where the mismatched numbers come from." },
      { q: "Who owns the workspace and the data?", a: "You do. It runs in your Microsoft tenant under your licences, and the model documentation is handed over with it rather than living in our heads." },
      { q: "How long before we see something?", a: "A working report in weeks, not quarters. An early version exists so you can argue with it, which is faster than specifying it in the abstract." },
      { q: "Will our team be able to maintain it?", a: "That is part of the deliverable. Measures are named and documented, and handover training sits in the estimate rather than arriving as an extra." },
      { q: "Is it only Power BI?", a: "Power BI is what these four systems are built in, so it is what we can prove. If your stack points somewhere else we will say so plainly rather than learning it on your budget." },
    ],
  },
  {
